  ...
  <div>
    ...
    >
      ...
      <div v-if="listType == 'picture-card' || listType == 'picture'">
        ...
        > -->
        <!-- v-else -->
        <el-button icon="el-icon-plus" size="small" style="border:none" plain />
      </div>
      <div v-if="isImg" slot="tip" class="el-upload__tip">
        只能上传图片格式文件<span v-if="!isMultiple">(只能上传一张)</span>
    ...
    <el-dialog :visible.sync="dialogVisible" width="30%" append-to-body>
      <img style="width:200px;margin:0 auto;" class="flex-center" :src="dialogImageUrl" alt="无效图片">
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  watch: {
    imgSrcStr: {
      handler(val, oldVal) {
        ...
        // this.fileList.push({ url: val, uid: Date.now() })
        // if (this.fileList.length > 1) {
        //   this.fileList.shift()
        //   console.log(this.fileList, 'this.fileList')
        // }
        // Vue.set(this.fileList, this.fileList[0], val)
        // this.fileList[0] = { url: val, uid: Date.now() }
        if (val) {
          this.fileList = [{ url: val, uid: Date.now() }]
        } else {
          this.fileList = []
        }
      },
    ...
    imgSrcList: {
      handler(val, oldVal) {
        if (this.isMultiple) {
          // console.log(val, '<======LIST====>', oldVal)
          if (val?.length > 0) {
            this.fileList = this.val.map(el => {
  ...
  methods: {
    ...
    handleRemove(file, fileList) {
      ...
      // console.log(file, fileList, this.fileList, this.fileList.findIndex(el => el.uid === file.uid))
      const index = this.fileList.findIndex(el => el.uid === file.uid)
      this.fileList.splice(index, 1)
      this.$emit('deleteFile', { file, fileList, index })
      // console.log(this.fileList)
    },
    // 预览图片
